...
def get_total_deduplicated_percentage(
    fastqc_data_open: IO, logger: logging.Logger
) -> list:
    line: str
...
def fastqc_summary_to_dict(
    data_dict: dict, fastqc_summary_path: str, logger: logging.Logger
) -> dict:
...
def fastqc_db(
    job_uuid: str, fastqc_zip_path: str, engine: engine, logger: logging.Logger
) -> None:
    ...
    summary_dict = fastqc_summary_to_dict(summary_dict, fastqc_summary_path, logger)
    ...
    for data_key in data_key_list:
        df = fastqc_detail_to_df(
            job_uuid, fastq_name, fastqc_data_path, data_key, logger
        )
        if df is None:
            continue
